...
// Моковые данные для карточек и заголовков статистики
const cards = [
    { title: 'Tasks', subtitle: '15 completed tasks', icon: 'list-bullet', iconFamily: 'Galio' },
    { title: 'Aquisitions', subtitle: '15 completed tasks', icon: 'bag-17', iconFamily: 'Galio' },
    { title: 'Cards', subtitle: '15 completed tasks', icon: 'credit-card', iconFamily: 'Galio' },
    { title: 'Settings', subtitle: '15 completed tasks', icon: 'settings-gear-65', iconFamily: 'Galio' },
];
const statsTitles = ['Jul', 'Aug', 'Sep', 'Oct', 'Nov'];

class Dashboard extends React.Component {
    // Рендеринг заголовка с навигационной панелью
    renderHeader = () => (
        <NavBar
            title="Dashboard"
            onLeftPress={() => this.props.navigation.openDrawer()} // Открытие бокового меню
            leftIconColor={theme.COLORS.MUTED}
            right={(
                <Button
                    color="transparent"
                    style={styles.settings}
                    onPress={() => this.props.navigation.openDrawer()} // Открытие бокового меню
                >
    ...
    // Рендеринг графиков статистики
    renderStats = () => {
        // GradientStats определяет градиент для графиков
        const GradientStats = () => (
            <Defs key="gradient">
        ...
        // Моковые данные для активной и неактивной статистики
        ...
        return (
            <Block style={{ marginBottom: BASE_SIZE * 3 }}>
                {/* Первый график с неактивными данными */}
                <AreaChart
                    yMin={0}
                ...
                {/* Второй график с активными данными */}
                <AreaChart
                    yMin={0}
                    yMax={Math.max(...statsActive) + 1}
                    data={statsActive}
    ...
    //метод создает список карточек, используя метод renderCard для каждой карточки
